...
def _reports_bucket(house: str, r: dict) -> str:
    ...
    use, so a session's metadata and its bodies move together."""
    try:
        if house == "ls":
            return bucket_for("ls|" + ls_file_id(
                int(r["lok_sabha"]), int(r["session"]), r["type"], int(r["question_no"])))
        return bucket_for("rs|" + rs_file_id(
            int(r["session"]), r["date_iso"], r["type"]))
